<script lang="ts">
	...
	let steuerElementeAus: boolean = false;
	let lastExecutionTime: number = 0;
	let timeoutHandle: ReturnType<typeof setTimeout> | null = null;

	onMount(() => {
		time = get(mediaLibrary).find((item) => item.tmdb.id === ID)?.watchTime || 0;
	});
	...
	function elemente(player: MouseEvent) {
		...
		timeoutHandle = setTimeout(() => {
			if (steuerElementeAus === false) {
				steuerElementeAus = true;
				mediaLibrary.update((library) => {
					return library.map((item) => {
						if (item.tmdb.id === ID) {
							if (item.watchTime > 0.75 * duration) {
								item.watched = true;
							}
							item.watchTime = Math.round(time);
						}
						return item;
					});
				});
...
<div
	class="relative z-0 flex justify-center select-none"
	bind:this={player}
	on:mousemove={(e) => {
		elemente(e);
	}}
>
	...
	<video
		...
		class="-z-50"
		{src}
		poster="{poster}}">The video tag does not support from you device.</video
	>
	<div
		class="absolute top-0 -z-40 h-full w-full transition-opacity ease-in-out duration-300"
		class:opacity-0={steuerElementeAus}
	>
		<div class="flex h-full w-full">
			...
			<div class="flex h-full w-2/12 place-items-center justify-center">
				<div class=" w-20 h-20 scale-150 flex place-items-center justify-center rounded-full">
					<label class="swap">
		...
		<!-- footer -->
		<div
			class="absolute bottom-0 h-fit w-full px-2 pt-1 from-black/90 bg-gradient-to-t to-transparent"
		>
